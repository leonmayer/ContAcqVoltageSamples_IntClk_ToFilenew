...
namespace NationalInstruments.Examples.ContAcqVoltageSamples_IntClk_ToFile
{
    ...
    public class MainForm : System.Windows.Forms.Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.writeToFileSaveFileDialog.FileOk += new System.ComponentModel.CancelEventHandler(this.writeToFileSaveFileDialog_FileOk);
            // 
        ...
        static List<string> timelist1 = new List<string>();
        static List<int> noofdatlist = new List<int>();

        static List<string> timelistfast = new List<string>();
        static List<double> datalistfast = new List<double>();
        ...
        private void browseWriteButton_Click(object sender, System.EventArgs e)
        {
            ...
            else
            {
                ...
                writeToFileSaveFileDialog.FileName = "acquisitionData_post.bin";
        ...
        private void LogData(double[,] data)
        {
            ...
            //noofdatlist.Add(dataCount);


            for (int i = 0; i < channelCount; i++)
            {
                ArrayList l = savedData[i] as ArrayList;
                //timelist1.Add($"{DateTime.Now:hh:mm:ss.FFF}");

                //my code
                //l.Add(Convert.ToDouble(channelCount));
                l.Add(Convert.ToDouble(dataCount));

                datalistfast.Add(Convert.ToDouble(dataCount));

                {
                    string datestring = $"{DateTime.Now:hh:mm:ss.fff}"; //01:53:15.82
                    double h1 = Char.GetNumericValue(datestring, 0);
                    double h2 = Char.GetNumericValue(datestring, 1);
                    double m1 = Char.GetNumericValue(datestring, 3);
                    double m2 = Char.GetNumericValue(datestring, 4);
                    double s1 = Char.GetNumericValue(datestring, 6);
                    double s2 = Char.GetNumericValue(datestring, 7);
                    double ms1 = Char.GetNumericValue(datestring, 9);
                    double ms2 = Char.GetNumericValue(datestring, 10);
                    double ms3 = Char.GetNumericValue(datestring, 11);
        ...
        {
            timerout = new Timer();
            timerout.Tick += new EventHandler(writerandtoout);
        ...
        private void CloseFile()
        {
            ...
            string[] timearray1 = timelist1.ToArray();
            ...
            try
            {
                if (useTextFileWrite)
                {
                    ...
                    for (int i = 0; i < dataCount; i++)
                    {
                        for (int j = 0; j < channelCount; j++)
                        {
                            ...
                            //fileStreamWriter.Write(timearray1[i]);
                            //my code
                ...
                else
                {
                    ...
                    /*
                    string ausgabe = fileBinaryReader.ReadString();
                    string test1 = ausgabe;
        ...
        private void PrepareFileForData()
        {
            ...
            else
            {
                ...
                fileBinaryWriterfast.Write(decimal.ToDouble(samplesPerChannelNumeric.Value));
                for (int i = 0; i < numChannels; i++)
                {
        ...
        private void writeToFileSaveFileDialog_FileOk(object sender, CancelEventArgs e)
        {

        }

        private void ReadBinaryDataString()
        {
            try
            {
                String s;
                ...
                string[,] array = new string[channelCount, samples];

                for (int iSample = 0; iSample < samples; iSample++)
                {
                    for (int iChan = 0; iChan < channelCount; iChan++)
                    {
                        array[iChan, iSample] = fileBinaryReader.ReadString();
                    }
                }

                InitializeDataTable(channels, ref dataTable);
                acquisitionDataGrid.DataSource = dataTable;
                DisplayDataString(array, ref dataTable);
            }
            catch (Exception e)
            {
                MessageBox.Show(e.ToString());
        ...
        private void DisplayDataString(string[,] sourceArray, ref DataTable dataTable)
        {
            //Display the first 10 points of the Read/Write in the Datagrid
            try
            {
